...
public:
    ...
    void push(T element, State* current) {
        int k = bucketNum(element);
        pthread_mutex_lock(&locks[k]);
        int altG;
        if (current == NULL) {
            ...
            altG = current->getLengthFromRoot() + 1;
        } 
        ...
        if (it != hashes[k].end()) {
            ...
            if (altG < element->getLengthFromRoot()) {
                element->setPrev(current);
                element->setLengthFromRoot(altG);
...
private:
    ...
    int bucketNum(T element) {
        //get hash value of element
        size_t hashval = stateHash()(element);
        return hashval % numThreads;
    }
